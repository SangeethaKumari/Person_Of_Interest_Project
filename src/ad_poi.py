...
# Use the directory where the script is located (moving up one level from src/)
...
max_images = 0               # Maximum images to process (0 = all images)

# AI Model configuration
MODELS = {
    "Base CLIP": "clip-ViT-B-32",
    "Enhanced CLIP-L": "clip-ViT-L-14",
...
class Siglip2Encoder:
    ...
    def get_sentence_embedding_dimension(self):
        # SigLIP 2 base uses 768
        return self.model.config.vision_config.hidden_size

def get_out_paths(model_key):
    """Generate model-specific paths for embeddings"""
    safe_name = model_key.lower().replace(" ", "_")
    return (
        data_dir / f"data/index_vectors_{safe_name}.npy",
        data_dir / f"data/index_meta.parquet" # Metadata (paths) remains shared
    )
...
def create_embeddings():
    """Create vector embeddings for all images using all configured models."""
    existing_images = sorted(list(img_dir.glob("*.[jJ][pP][gG]")))
    
    if not existing_images:
        print("❌ No images found to index. Please add images to docs/images/celebA/")
        return

    # First, ensure shared metadata exists
    _, meta_path = get_out_paths(next(iter(MODELS)))
    if not meta_path.exists():
        print("📝 Creating shared metadata...")
        pd.DataFrame({"path": [str(p.relative_to(data_dir)) for p in existing_images]}).to_parquet(meta_path, index=False)

    for model_name, model_id in MODELS.items():
        vec_path, _ = get_out_paths(model_name)
        
        if vec_path.exists():
            print(f"✅ {model_name} embeddings already exist. Skipping.")
            continue
        
        print(f"🧠 Generating embeddings using {model_name} ({model_id})...")
        
        # Load the specific model
        if "siglip" in model_id.lower():
            model = Siglip2Encoder(model_id)
        else:
            model = SentenceTransformer(model_id)
        
        # Process all images
        vecs = []
        for p in tqdm(existing_images, desc=f"Indexing {model_name}"):
            try:
                ...
                # Append a zero vector so indices stay aligned with metadata
                vecs.append(np.zeros(model.get_sentence_embedding_dimension()))
        
        # Save embeddings
        np.save(vec_path, np.vstack(vecs))
        print(f"✅ Indexed {len(vecs)} images for {model_name}.")
...
def load_all_models_and_data():
    ...
    # Load basic metadata once (shared)
    _, meta_path = get_out_paths(next(iter(MODELS)))
    if not meta_path.exists():
        st.error("❌ Metadata not found! Please run the indexing script first.")
        st.stop()
    ...
    for model_name, model_id in MODELS.items():
        ...
        if not vec_path.exists():
            st.error(f"❌ Embeddings for {model_name} not found! Please run indexing.")
            st.stop()
...
def run_search_app():
    """Launch the Streamlit search interface with model comparison."""
    # Load models and data (cached)
    with st.spinner("🚀 Loading AI Models (this may take a minute first time)..."):
        model_data, meta = load_all_models_and_data()
    ...
    with col2:
        st.metric("🤖 Model A", "Base CLIP (ViT-B-32)")
    with col3:
        st.metric("🚀 Model B", "Enhanced CLIP-L (ViT-L-14)")
    with col4:
        st.metric("🎯 Model C", "SigLIP 2 (Google)")
    ...
    with col1:
        ...
        # --- EXPERIMENTAL: CELEBRITY DOPPELGÄNGER ---
        st.markdown("""
        <div style="background-color: #f0f4f8; padding: 20px; border-radius: 15px; border-left: 10px solid #2c5aa0; margin-bottom: 25px;">
            <h2 style="color: #2c5aa0; margin-top: 0;">🎭 Celebrity Doppelgänger (Upload Search)</h2>
            <p style="color: #4a5568;">Upload a photo of yourself or anyone else to find their closest matches in our celebrity database.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if uploaded_file is not None:
            ...
            st.markdown("### 🔍 Searching for matches for:")
            st.image(user_img, width=250, caption="Your Uploaded Photo")
            
            # Perform image-to-image search across all models
            doppel_cols = st.columns(3)
            
            with st.spinner("AI is analyzing your features..."):
                for i, (model_name, data) in enumerate(model_data.items()):
                    target_col = doppel_cols[i]
                    with target_col:
                        st.markdown(f"#### 🛡️ {model_name} Matches")
                        
                        # 1. Image Embedding
                        model = data["model"]
                        vecs = data["vecs"]
                        
                        # SigLIP 2 vs CLIP encoding
                        img_vec = model.encode(user_img, convert_to_numpy=True, normalize_embeddings=True).reshape(1, -1)
                        
                        # 2. Similarity
                        sims = cosine_similarity(img_vec, vecs).ravel()
                        top = sims.argsort()[::-1][:3] # Top 3 doppelgängers
                        
                        # 3. Display
                        for idx in top:
                            score = float(sims[idx])
                            # Map image-to-image score (usually higher than text)
                            display_score = min(0.99, (score * 1.5))
                            
                            img_path_raw = meta.loc[idx, "path"]
                            img_path = data_dir / img_path_raw
                            if not img_path.exists():
                               img_path = data_dir / "docs/images/celebA" / os.path.basename(img_path_raw)
                            
                            st.markdown(f"""
                            <div style="border: 1px solid #7FB3D3; border-radius: 10px; padding: 5px; background: white; text-align: center;">
                                <span style="font-weight: bold; color: #2c5aa0;">{display_score:.1%} Match</span>
                            </div>
                            """, unsafe_allow_html=True)
        ...
        # --- TEXT SEARCH SECTION ---
        st.markdown("""
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 15px; border-left: 10px solid #7FB3D3; margin-bottom: 25px;">
            <h2 style="color: #2c5aa0; margin-top: 0;">🔍 Narrative Search</h2>
            <p style="color: #6c757d;">Search images using natural language descriptions (e.g., 'woman with blonde hair and blue eyes').</p>
        </div>
        """, unsafe_allow_html=True)
        
        # Main search bar with focus
        q = st.text_input("Describe the person you are looking for:", 
                          placeholder="e.g., Person with blonde hair and blue eyes",
                          key="search_input")
    ...
    if q:
        ...
        with st.spinner("AI is analyzing images..."):
            for i, (model_name, data) in enumerate(model_data.items()):
                target_col = results_cols[i]
                with target_col:
                    st.markdown(f"### 🛡️ {model_name} Results")
                    ...
                    for idx in top:
                        score = float(sims[idx])
                        # Map score to higher display percentage for better UX
                        # (0.3 -> ~85% feel) using a simple scaling
                        display_score = min(0.99, (score * 2.5) + 0.1) 
                        
                        img_path_raw = meta.loc[idx, "path"]
                        # Resolve path
                        img_path = data_dir / img_path_raw
                        if not img_path.exists():
                            # Fallback for old abs paths
                            img_path = data_dir / "docs/images/celebA" / os.path.basename(img_path_raw)

                        # Result Card
                        st.markdown(f"""
                        <div style="border: 2px solid #7FB3D3; border-radius: 15px; padding: 10px; margin-bottom: 10px; background: white;">
                            <p style="margin:0; font-weight: bold; color: #2c5aa0;">Match Confidence: {display_score:.1%}</p>
                        </div>
                        """, unsafe_allow_html=True)
